...
#include "../Utilities/GraphUtilsSingleForeground.hpp"
...
class GraphFactory
{
  ...
  public:
    ...
    GraphData *create(LCADB *lcadb, vector<long> procs)
    {
        ...
        (*graph).nodesInfoMap[settings.RootProcessId].scalar=abs(settings.OutputQunatity);

        GraphUtilsSingleForeground graphutil(lcadb, graph, settings);
        ...
        std::cout << "g_foreground/c++," << settings.ProjectId << "," << elapsed_gr.count() << std::endl;
