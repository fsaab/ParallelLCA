...
class MontecarloCalculator
{
  public:
    static void run(AppSettings settings,
                    ...
                    LCADB *lcadb, IterationsResults *iterationsResults, GraphData *graph)
    {
        ...
        for (int i = 0; i < parallelism; i++)
        {
            ...
            AggregatedSimulator simulator(calculatorData, settings);

            GraphData g = (*graph);

            for (auto &&k_edge : g.edgesListCompact)
            {
                k_edge.second.InputScalar = &(g.nodesInfoMap[k_edge.second.ProcessSrcId]);
                k_edge.second.OutputScalar = &(g.nodesInfoMap[k_edge.second.ProcessDestId]);
            }

            for (auto &&k_edge : g.edgesStack)
            {

                for (auto &&ed :k_edge.second)
                {
                  ed.InputScalar = &(g.nodesInfoMap[ed.ProcessSrcId]);
                  ed.OutputScalar = &(g.nodesInfoMap[ed.ProcessDestId]);
                }

            }

            IterationsResults *iterationResults_i = simulator.iterate_aggreagtedDS(iterationOffset,
            ...
            #pragma omp critical
            {
                ...
                while (iter <= iterationOffset + threadSize - 1)
                {
                    ...
                    //cout<<"store_lcia_ptr size :"<<((*iterationResults_i).store_lcia_ptr)[iter - iterationOffset].size()<<endl;

                    // for(int i=0;i<settings.montecarlo_iterations;i++) cout<< ((*iterationResults_i).store_lcia_ptr)[iter - iterationOffset][i]<<" , ";
                    // cout<<endl;
                     ...
                     LCADB *lcadb)
    {
        ...
        //cout<<"impcatsLength "<<impcatsLength<<endl;
        ...
        //cout<<"(*iterationsResults).store_lcia_ptr "<<(*iterationsResults).store_lcia_ptr.size()<<endl;
        ...
        for (int categoryid = 0; categoryid < impcatsLength; categoryid++)
        {
            ...
            // for(int i=0;i<settings.montecarlo_iterations;i++) cout<< ptr_lcia[i]<<" ,, ";
            // cout<<endl;
            ...
            //cout<<"(*calculatorData).lcaIndexes.ImpactCategoryIndex size "<<(*calculatorData).lcaIndexes.ImpactCategoryIndex.size()<<endl;
